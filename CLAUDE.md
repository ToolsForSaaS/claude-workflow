...
- Les fichiers dans `commands/` et `skills/` sont **partagés** — ils sont copiés tels quels dans le `.claude/` de chaque projet
...
- Les chemins dans les commandes doivent toujours utiliser `.claude/skills/...` (chemin relatif au projet, pas au workflow repo)

## Structure

```
commands/     → copiés dans .claude/commands/ de chaque projet
skills/       → copiés dans .claude/skills/ de chaque projet
