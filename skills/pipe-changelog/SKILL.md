...
6. **Reformuler** —
   ...
   - Appliquer les **regles de redaction** (sous-section "Regles de redaction" de `reference.md`) sur chaque entree : voix active au present, API publique impactee precisee entre backticks, breaking changes prefixes `**BREAKING**`. Le **template mental** `[Verbe actif present] [symbole/feature publique] [effet visible utilisateur] [migration si breaking]` aide a structurer la formulation.
   - Pour TECHNICAL : entree concise orientee contributeur (ce qui a change dans le repo), memes regles de redaction, meme regle d'une seule ligne et meme reference tracable.
